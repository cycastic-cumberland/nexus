...
private:
    ...
    void free_data();
    void clear_and_set_from(const void* p_data, NexusSerializedBytecode::DataType p_data_type);
    ...
    template<class T>
    _FORCE_INLINE_ T*& get_internal(const NexusSerializedBytecode::DataType& p_type){
...
public:
    ...
    AmbiguousValue(void* p_data, NexusSerializedBytecode::DataType p_type) {
        clear_and_set_from(p_data, p_type);
    }
